...
public class SCCAlgorithm implements SCCAlgorithmInterface {
    ...
    private int[] nodeFinishedAt; // Array of node indexes in graph indexed by
                                  // finish time
    ...
    /**
     * @param Graph
     *            to search for strongly connected components.
     */
    ...
    public void executeAlgorithm() {
        performDFS();
        reversedGraph = theGraph.getTranspose(); // Clones the graph in
                                                 // O(|E|+|V|) time with
                                                 // direction of edges reversed
        performReversedDFS();

    }

    /**
     * @return the ArrayList<ArrayList<GraphNode>> representation of the
    ...
    /**
     * Performs the 1st DFS and records discovery times in each node. Places
     * each node in Array indexed by discovery time.
     */
    private void performDFS() {
        int finishTime = 0; // Finish time starts at 0
        int minUnvisited = 0; // Lowest index of an unvisited node
        List<GraphNode> allNodes = theGraph.getAllNodes(); // get list of nodes
                                                           // from the graph

        while (finishTime < allNodes.size() && minUnvisited < allNodes.size()) { // While
                                                                                 // we
                                                                                 // haven't
                                                                                 // discovered
                                                                                 // all
            ...
            while (minUnvisited < allNodes.size()
                    && allNodes.get(minUnvisited).getColor() != GraphNode.Color.UNVISITED)
                minUnvisited++;

            // If we haven't visited all nodes, then visit the next unvisited
            // one
            if (minUnvisited < allNodes.size())
                finishTime = visitAndRecordFinish(allNodes.get(minUnvisited),
                        finishTime);
    ...
    private void performReversedDFS() {
        int maxFinishTime = nodeFinishedAt.length - 1; // Highest finish time of
                                                       // an unvisited node
        List<GraphNode> allNodes = reversedGraph.getAllNodes(); // Get list of
                                                                // nodes from
                                                                // the REVERSED
                                                                // graph
        ...
        while (maxFinishTime >= 0) { // While we haven't visited all nodes
            ...
            // Get the index of a remaining unvisited node with the highest
            // finish time
            while (maxFinishTime >= 0
                    && allNodes.get(nodeFinishedAt[maxFinishTime]).getColor() != GraphNode.Color.UNVISITED)
                maxFinishTime--;

            // If we haven't visited all nodes, start a new strongly connected
            // component, and visit the next one
            if (maxFinishTime >= 0) {
                sCCs.add(new ArrayList<GraphNode>());
                visitAndAddToComponent(allNodes
                        .get(nodeFinishedAt[maxFinishTime]));
            }
        }
    }
     ...
     * Returns the finish time to avoid having more member variables than
     * necessary.
     */
    private int visitAndRecordFinish(GraphNode node, int time) { // Returns new
                                                                 // time value
    ...
    /**
     * Recursive visit function that builds the strongly connected components
     * for the 2nd DFS
     */
    ...
    /**
     * Return a string representing the strongly connected components, sorted
     * for readability.
     * 
     * @return a String representing the strongly connected components of the
     *         graph.
     */
    public String componentsToString() {
        for (int i = 0; i < sCCs.size(); i++) { // Sort each Connected Component
                                                // for easy viewing
